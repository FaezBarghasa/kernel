...
pub fn clock_gettime(clock_id: usize, time: &mut time::TimeSpec) -> Result<usize> {
    match clock_id {
        CLOCK_REALTIME => {
            ...
            time.tv_sec = (realtime / 1_000_000_000) as i64;
            time.tv_nsec = (realtime % 1_000_000_000) as i64;
            Ok(0)
        }
        CLOCK_MONOTONIC => {
            let monotonic = crate::time::monotonic();
            time.tv_sec = (monotonic / 1_000_000_000) as i64;
            time.tv_nsec = (monotonic % 1_000_000_000) as i64;
            Ok(0)
        }
